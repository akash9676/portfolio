        ...
        <div className="h-screen flex flex-col text-white relative">
            ...
            <div className=" flex items-center justify-center absolute h-screen w-full opacity-85 rounded-md">
                ...
                animate = {{ type : "easeinout" }}
                    >
                    <motion.img className="w-52 h-52 rounded-full max-sm:mb-4 self-center md:h-[300px] object-cover md:w-max lg:rounded-tl-md lg:rounded-bl-md md:rounded-none" src={profile} alt="" 
                    initial={{ opacity: 0 , x: -25,scale: 0.8 }}
                    whileInView={{ opacity: 0.9 , x: 0,scale: 1 }}
                    transition={{duration:0.5}}
                    animate = {{ type : "easeinout" }}
                    />
                    <motion.div className="px-6 py-5 flex flex-col justify-center bg-about max-sm:rounded-md sm:rounded-tr-md sm:rounded-br-md"
                    initial={{ opacity: 0 , x: 25,scale: 0.8 }}
                    whileInView={{ opacity: 0.9 , x: 0,scale: 1 }}
                    transition={{duration:0.5}}
                    animate = {{ type : "easeinout" }}
                    >
                        <motion.div className="text-intro text-2xl md:text-4xl font-medium lg:mb-3"
